...
<body class="bg-[#F5F5F5] font-sans text-[#111111]">
        ...
        <div class="container-magazine">
            <div class="flex items-center justify-between h-14 md:h-16">
                ...
                <div class="flex items-center gap-6 text-sm font-medium text-gray-500">
                    <a href="{{ route('driver.schedule') }}" class="hover:text-[#C1121F] transition {{ request()->routeIs('driver.schedule') ? 'text-[#C1121F]' : '' }}">Jadwal</a>
                    <a href="{{ route('driver.profile') }}" class="hover:text-[#C1121F] transition {{ request()->routeIs('driver.profile') ? 'text-[#C1121F]' : '' }}">Profil</a>
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="text-gray-400 hover:text-[#C1121F] transition font-medium">Keluar</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    {{-- BOTTOM NAV MOBILE --}}
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-[#E5E5E5] md:hidden z-50">
        <div class="flex items-center justify-around py-2">
            <a href="{{ route('driver.schedule') }}" class="flex flex-col items-center gap-1 text-[10px] {{ request()->routeIs('driver.schedule') ? 'text-[#C1121F]' : 'text-gray-500' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                <span>Jadwal</span>
            </a>
            <a href="{{ route('driver.profile') }}" class="flex flex-col items-center gap-1 text-[10px] {{ request()->routeIs('driver.profile') ? 'text-[#C1121F]' : 'text-gray-500' }}">

    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#E5E5E5] transform transition-transform duration-300 lg:relative lg:translate-x-0 overflow-y-auto"
            ...
            <nav class="p-3 space-y-1">
                @if(!$hasAgent)
                <a href="{{ route('payment-agent.setup') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-sm transition {{ request()->routeIs('payment-agent.setup') ? 'bg-[#C1121F]/10 text-[#C1121F] font-semibold' : 'text-gray-600 hover:bg-[#F5F5F5]' }}">
                    <span>📝</span> Setup Warung
                </a>
                @else
                <a href="{{ route('payment-agent.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-sm transition {{ request()->routeIs('payment-agent.dashboard') ? 'bg-[#C1121F]/10 text-[#C1121F] font-semibold' : 'text-gray-600 hover:bg-[#F5F5F5]' }}">
                    <span>📊</span> Dashboard
                </a>
                
                @if($agent->is_verified)
                <a href="{{ route('payment-agent.payments') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-sm transition {{ request()->routeIs('payment-agent.payments') ? 'bg-[#C1121F]/10 text-[#C1121F] font-semibold' : 'text-gray-600 hover:bg-[#F5F5F5]' }}">
                    <span>💰</span> Pembayaran
                </a>
                <a href="{{ route('payment-agent.settlements') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-sm transition {{ request()->routeIs('payment-agent.settlements') ? 'bg-[#C1121F]/10 text-[#C1121F] font-semibold' : 'text-gray-600 hover:bg-[#F5F5F5]' }}">
                    <span>📋</span> Settlement
                </a>
                @endif
                
                <a href="{{ route('payment-agent.profile') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-sm transition {{ request()->routeIs('payment-agent.profile') ? 'bg-[#C1121F]/10 text-[#C1121F] font-semibold' : 'text-gray-600 hover:bg-[#F5F5F5]' }}">
                    <span>⚙️</span> Profil
                </a>
                @endif
            </nav>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto p-4 lg:p-6">
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-[12px] p-4 mb-6 flex items-center justify-between">
                    ...
                    <a href="{{ route('payment-agent.profile') }}" class="bg-[#C1121F] text-white px-4 py-2 rounded-[12px] text-sm font-medium hover:bg-[#8A0F18] transition whitespace-nowrap ml-4">Lihat Profil</a>
                </div>
                @endif

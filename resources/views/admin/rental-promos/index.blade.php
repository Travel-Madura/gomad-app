...
<div>
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 border-b border-[#E5E5E5] pb-3">
        ...
        <a href="{{ route('admin.rental-promos.create') }}" class="btn-gomad-primary text-sm inline-flex items-center gap-2 self-start rounded-[12px] px-4 py-2.5">
            + Buat Promo Rental
        </a>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-[#E5E5E5]">
                    ...
                    <tr class="hover:bg-[#F5F5F5]">
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-mono uppercase tracking-wider border
                                ...
                                @else bg-purple-50 text-purple-700 border-purple-200 @endif">{{ $promo->type_label }}</span>
                        </td>
                        <td class="px-4 py-3 text-center font-bold text-[#C1121F]">{{ $promo->discount_percent }}%</td>
                        <td class="px-4 py-3 text-[10px] font-mono text-gray-500">{{ $promo->start_date->format('d M') }} - {{ $promo->end_date->format('d M Y') }}</td>
                        <td class="px-4 py-3 text-center">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.rental-promos.edit', $promo) }}" class="text-[#C1121F] hover:underline text-xs font-medium mr-2">Edit</a>
                            <form action="{{ route('admin.rental-promos.destroy', $promo) }}" method="POST" class="inline" onsubmit="return confirm('Hapus?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500 font-light">Belum ada promo rental.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

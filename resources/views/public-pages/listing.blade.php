...
<div class="section">
    <div class="container-magazine">
        <div class="mb-8">
            <h1 class="text-3xl md:text-4xl font-bold text-[#111111] mb-2">Daftar Agency Travel</h1>
            <p class="text-gray-500 font-light">Perkenalkan agency travel andalan kami.</p>
        </div>

        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8 border-b border-[#E5E5E5] pb-4">
            <form action="{{ route('listing') }}" method="GET" class="flex-1 max-w-md">
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari agency..." 
                           class="w-full pl-10 pr-4 py-3 border-b-2 border-[#E5E5E5] focus:border-[#C1121F] outline-none bg-transparent font-medium text-[#111111] placeholder-gray-400 transition-colors">
            ...
            <div class="flex bg-[#F5F5F5] rounded-lg p-1">
                <a href="{{ request()->fullUrlWithQuery(['view' => 'grid']) }}" 
                   class="px-3 py-1.5 rounded-md text-sm {{ $viewMode == 'grid' ? 'bg-white shadow text-[#C1121F] font-medium' : 'text-gray-500 hover:text-[#111111]' }}">
                    <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                </a>
                <a href="{{ request()->fullUrlWithQuery(['view' => 'list']) }}" 
                   class="px-3 py-1.5 rounded-md text-sm {{ $viewMode == 'list' ? 'bg-white shadow text-[#C1121F] font-medium' : 'text-gray-500 hover:text-[#111111]' }}">
        ...
        @else
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <a href="{{ route('agency.profile', $agency->slug) }}" class="card-gomad overflow-hidden group block border-[#E5E5E5] hover:border-[#C1121F] p-0">
                    <div class="h-32 bg-[#F5F5F5] flex items-center justify-center overflow-hidden border-b border-[#E5E5E5]">
                    ...
                    <div class="p-5">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-12 h-12 rounded-full border-2 border-white -mt-10 bg-white shadow overflow-hidden flex-shrink-0">
                                ...
                                @else <div class="w-full h-full bg-[#F5F5F5] flex items-center justify-center text-lg">🏢</div> @endif
                            </div>
                            <div class="pt-2">
                                <h3 class="font-bold text-[#111111] group-hover:text-[#C1121F] transition">{{ $agency->agency_name }}</h3>
                                @if($agency->is_verified)<span class="text-xs text-[#C1121F] font-mono uppercase tracking-wider">Terverifikasi</span>@endif
                            </div>
                        </div>
                        <p class="text-sm text-gray-500 line-clamp-2 font-light mb-3">{{ $agency->description ?? 'Belum ada deskripsi.' }}</p>
                        <div class="flex items-center justify-between text-sm border-t border-[#E5E5E5] pt-3">
            ...
            <div class="space-y-4">
                @foreach($agencies as $agency)
                <a href="{{ route('agency.profile', $agency->slug) }}" class="card-gomad p-5 flex flex-col sm:flex-row items-start gap-4 group border-[#E5E5E5] hover:border-[#C1121F]">
                    <div class="w-16 h-16 rounded-[12px] bg-[#F5F5F5] flex items-center justify-center text-2xl overflow-hidden flex-shrink-0">
                        @if($agency->logo)<img src="{{ $agency->logo }}" alt="" class="w-full h-full object-cover">@else 🏢 @endif
                    </div>
                    <div class="flex-1">
                        <div class="flex items-center gap-2">
                            <h3 class="font-bold text-[#111111] group-hover:text-[#C1121F] transition">{{ $agency->agency_name }}</h3>
                            @if($agency->is_verified)<span class="text-xs text-[#C1121F] font-mono uppercase tracking-wider">✓</span>@endif
                        </div>

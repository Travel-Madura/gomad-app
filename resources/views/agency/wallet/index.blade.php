...
<div>
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 border-b border-[#E5E5E5] pb-3">
        <h1 class="text-2xl font-bold text-[#111111]">Dompet Agency</h1>
        <a href="{{ route('agency.wallet.topup') }}" class="btn-gomad-primary text-sm inline-flex items-center gap-2 self-start rounded-[12px]">
    ...
    <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-5 mb-6 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-[#111111]">Status COD</h3>
            ...
            <a href="{{ route('agency.wallet.topup') }}" class="bg-[#C1121F] text-white px-4 py-2 rounded-[12px] text-sm font-medium hover:bg-[#8A0F18] transition whitespace-nowrap">Top Up</a>
        </div>
    </div>

    {{-- Transfer ke Deposit --}}
    <div class="bg-white border border-green-200 rounded-[12px] p-6 mb-6 shadow-sm">
        <div class="flex items-center gap-3 mb-4">
            <div class="w-10 h-10 rounded-full bg-green-50 border border-green-200 flex items-center justify-center text-lg">🔄</div>
            <div>
                <h3 class="font-bold text-[#111111]">Transfer ke Saldo Deposit</h3>
        ...
        <form action="{{ route('agency.wallet.transfer-deposit') }}" method="POST" class="flex flex-col sm:flex-row gap-3">
            @csrf
            <div class="flex-1">
                <input type="number" name="amount" class="w-full px-0 py-2 border-b-2 border-[#E5E5E5] focus:border-[#C1121F] outline-none bg-transparent text-[#111111] transition" placeholder="Nominal transfer" min="10000" max="{{ (int) $balanceSummary['available_balance'] }}" required>
            ...
            <button type="submit" class="bg-[#C1121F] text-white px-6 py-3 rounded-[12px] font-semibold hover:bg-[#8A0F18] transition whitespace-nowrap">Transfer</button>
        </form>
    </div>
    ...
    <div class="flex flex-wrap gap-3 mb-8">
        ...
        <a href="{{ route('agency.wallet.topup') }}" class="bg-[#C1121F] text-white px-6 py-3 rounded-[12px] text-sm font-semibold hover:bg-[#8A0F18] transition inline-flex items-center gap-2">
    ...
    <div class="flex gap-2 mb-4 overflow-x-auto pb-2">
        <a href="{{ route('agency.wallet.index') }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ !request('type') ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">Semua</a>
        <a href="{{ route('agency.wallet.index', ['type' => 'topup']) }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ request('type') == 'topup' ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">Top Up</a>
        <a href="{{ route('agency.wallet.index', ['type' => 'booking']) }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ request('type') == 'booking' ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">Booking</a>
        <a href="{{ route('agency.wallet.index', ['type' => 'cod_schedule_hold']) }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ request('type') == 'cod_schedule_hold' ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">COD Hold</a>
        <a href="{{ route('agency.wallet.index', ['type' => 'cod_schedule_release']) }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ request('type') == 'cod_schedule_release' ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">COD Release</a>
        <a href="{{ route('agency.wallet.index', ['type' => 'transfer_to_deposit']) }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ request('type') == 'transfer_to_deposit' ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">Transfer</a>
        <a href="{{ route('agency.wallet.index', ['type' => 'withdrawal']) }}" class="px-3 py-1.5 rounded-[8px] text-[10px] font-mono uppercase tracking-wider font-medium whitespace-nowrap border {{ request('type') == 'withdrawal' ? 'bg-[#C1121F] text-white border-[#C1121F]' : 'bg-[#F5F5F5] text-gray-600 border-[#E5E5E5]' }}">Penarikan</a>
    </div>

    {{-- Riwayat Transaksi --}}
    <div>
        <h2 class="font-bold text-lg text-[#111111] mb-4">Riwayat Mutasi Saldo</h2>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-[#E5E5E5]">
                        ...
                        <tr class="hover:bg-[#F5F5F5]">
                            <td class="px-4 py-3">
                                <span class="text-[#111111]">{{ $t->description }}</span>
                            ...
                            <td class="px-4 py-3 text-right font-mono text-[#111111]">Rp {{ number_format($t->balance_after, 0, ',', '.') }}</td>

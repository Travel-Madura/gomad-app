...
<div>
    <h1 class="text-2xl font-bold text-[#111111] mb-6">Promo Tersedia</h1>

    {{-- Promo Selektif --}}
    <div class="mb-8">
        <h2 class="font-bold text-lg text-[#111111] mb-4">Promo Selektif</h2>
        @if($promos->isEmpty())
        <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-8 text-center text-gray-500 shadow-sm font-light">Belum ada promo selektif.</div>
        @else
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-5 border-l-4 border-[#C1121F] shadow-sm">
                ...
                <p class="text-xs text-gray-500 font-mono tracking-wider">💰 {{ $promo->cost_bearer_label }}</p>
            </div>
            @endforeach
        </div>
        @endif
    ...
    <div>
        <h2 class="font-bold text-lg text-[#111111] mb-4">Pasang Promo ke Jadwal</h2>
        @if($schedules->isEmpty())
        <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-8 text-center text-gray-500 shadow-sm font-light">Belum ada jadwal.</div>
        @else
        <div class="space-y-3">
            ...
            <div class="bg-white border border-[#E5E5E5] rounded-[12px] p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 shadow-sm">
                <div>
                    <span class="font-semibold text-[#111111]">{{ $schedule->route->route_name }}</span>
                    <span class="text-sm text-gray-500 ml-2 font-light">{{ $schedule->departure_date->format('d M Y') }} {{ $schedule->departure_time }}</span>
                    @if($schedule->promos->isNotEmpty())
                    <span class="text-[10px] font-mono uppercase tracking-wider bg-green-50 text-green-700 px-2 py-1 rounded-full border border-green-200 ml-2">{{ $schedule->promos->first()->name }}</span>
                    @endif
                </div>
                ...
                <form action="{{ route('agency.promos.attach') }}" method="POST" class="flex gap-2">
                    ...
                    <select name="promo_id" class="text-xs border border-[#E5E5E5] rounded-[12px] px-2 py-1.5 bg-[#F5F5F5] text-[#111111]">
                        @foreach($promos as $promo)
                        <option value="{{ $promo->id }}">{{ $promo->name }} ({{ $promo->discount_percent }}%)</option>
                        @endforeach
                    </select>
                    <button type="submit" class="bg-[#C1121F] text-white px-3 py-1.5 rounded-[12px] text-[10px] font-medium hover:bg-[#8A0F18]">Pasang</button>
                </form>
                @endif
            </div>
            @endforeach
